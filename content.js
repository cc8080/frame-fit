 ...
 *
 * A snackbar will briefly appear to indicate "done" or "nothing found to do".
 *
 */
...
if (typeof window.frameFitInjected === 'undefined') {
  ...
  //const bgBlue = 'rgba(173, 216, 230, 0.5)'; 
  const bgGreen = 'rgba(152,251,152, 0.95)';
  const bgYellow = 'rgba(255, 255, 0, 0.95)';
  ...
  function askForResizeIfNotTop() {
    ...
    if (window.parent !== window) {
      ...
      // At the top, so we're done. Show a snackbar
      snackTime("Frame-fit: Done", 2000, bgGreen);
    }
  }
  ...
  function snackTime(whadIDo, howLong, color) {
    // show a message, for howLong milliseconds, with color backgroundcolor
    
    howLong = ( howLong < 1000 ) ? 1000 : howLong; // minimum is 1000 milliseconds
    const timeoutSecs = howLong/1000;
    
    snackDiv.innerHTML = whadIDo; // the message to show
    snackDiv.style.backgroundColor = color;
    
    // Add animation: Take 0.5 seconds to fade in and out the snackbar. 
    // However, delay the fade out process for timeoutSecs seconds
    ...
    // After some seconds, hide the snackbar again
    setTimeout(function(){ 
